...
type ManageRemarkTypesModalProps = Readonly<{
  open: boolean;
  options: RemarkTypeOption[];
  /** Company-wide remark types available to copy when adding a new one. */
  companyOptions?: RemarkTypeOption[];
  onClose: () => void;
  onSave: (options: RemarkTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Remark Type";

function optionToDraft(option: RemarkTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
...
function draftToOption(draft: DraftForm, id: string): RemarkTypeOption {
...
function validateName(
  ...
  if (!trimmed) return "Remark type name is required.";
  ...
  if (duplicate) return "A remark type with this name already exists.";
  return undefined;
}
...
export function ManageRemarkTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageRemarkTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<RemarkTypeOption[]>([]);
  ...
  const aliasSelectOptions = useMemo(
    () => REMARK_TYPE_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );

  const resetPanel = useCallback(
    (nextSelectedId: string | null, adding: boolean, nextEntries: RemarkTypeOption[]) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseRemarkTypeOptions(options, []);
  ...
  const commitDraft = useCallback((): RemarkTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} remark types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("remark-type"))];
    }
  ...
  const handleSelectEntry = (entry: RemarkTypeOption) => {
  ...
  const persistEntries = async (nextEntries: RemarkTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Remark type name is required." });
        return;
      }
  ...
  const listLabelForEntry = (entry: RemarkTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Remark Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-remark-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-remark-types-title" className="project-modal__title">
              Manage Remark Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing remark types by selecting from the left menu. You can
              also drag to reorder. You also have the option to create a new Remark Type or
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  <div className="project-modal__split-sidebar-head">
                    <span className="project-modal__split-sidebar-label">Remark Types</span>
                    <UiButton
                      type="button"
                      variant="primary"
                      size="sm"
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new Remark Type, or copy one used elsewhere in your company
                      account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Remark Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Remark Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Remark Type from scratch
                        </p>
      ...
      <ConfirmDialog
        ...
        message={
          selectedEntry
            ...
            : "This will remove the selected remark type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
