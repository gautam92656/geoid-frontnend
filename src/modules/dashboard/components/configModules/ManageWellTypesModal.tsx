...
import {
  ...
  type WellTypeOption,
} from "../../utils/configModules/wellType";
import { listWellTypeGraphics } from "../../services/wellTypeGraphicsApi";
import { SelectDrillingGraphicsModal } from "./SelectDrillingGraphicsModal";

type ManageWellTypesModalProps = Readonly<{
  open: boolean;
  options: WellTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WellTypeOption[];
  onClose: () => void;
  onSave: (options: WellTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Well Type";

function optionToDraft(option: WellTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
      graphic: DEFAULT_WELL_TYPE_GRAPHIC,
  ...
  return {
    ...
    graphic: option.graphic ?? DEFAULT_WELL_TYPE_GRAPHIC,
    allowNegativeDepth: Boolean(option.allowNegativeDepth),
  };
}

function draftToOption(draft: DraftForm, id: string): WellTypeOption {
  return createBlankWellTypeOption({
    id,
    name: draft.name.trim(),
    tablogsAlias: draft.tablogsAlias.trim() || null,
    graphic: draft.graphic.trim() || DEFAULT_WELL_TYPE_GRAPHIC,
    allowNegativeDepth: draft.allowNegativeDepth,
  });
}

function reorderEntries(
  entries: WellTypeOption[],
  sourceId: string,
  targetId: string
): WellTypeOption[] {
...
function validateName(
  ...
  if (!trimmed) return "Well type name is required.";
  ...
  if (duplicate) return "A well type with this name already exists.";
  return undefined;
}
...
export function ManageWellTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWellTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WellTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WellTypeGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWellTypeGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WELL_TYPE_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );

  const fallbackGraphics = useMemo(
    () => FALLBACK_WELL_TYPE_GRAPHICS.map((filename) => toWellTypeGraphicCatalogEntry(filename)),
    []
  );

  const resetPanel = useCallback(
    (nextSelectedId: string | null, adding: boolean, nextEntries: WellTypeOption[]) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWellTypeOptions(options, []);
  ...
  useEffect(() => {
    ...
    listWellTypeGraphics()
      .then((catalog) => {
        if (cancelled) return;
        const fromApi = (catalog.graphics ?? []).map((entry) =>
          toWellTypeGraphicCatalogEntry(entry.filename, entry.label)
        );
      ...
      .catch((error: unknown) => {
        ...
        setGraphicsError(null);
        console.warn(error instanceof Error ? error.message : "Failed to load well type graphics.");
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WellTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} well types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("well-type"))];
    }
  ...
  const handleSelectEntry = (entry: WellTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WellTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Well type name is required." });
        return;
      }
  ...
  const listLabelForEntry = (entry: WellTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Well Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-well-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-well-types-title" className="project-modal__title">
              Manage Well Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing well types by selecting from the left menu. You can also drag to
              reorder. You also have the option to create a new Well Type or copy an existing one
            ...
            <div className="project-modal__body ui-scrollbar">
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul className="project-modal__type-list" role="listbox" aria-label="Well types">
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">No well types yet.</li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new well type, or copy one used elsewhere in your company account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Well Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Well Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Well Type from scratch
                        </p>
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={wellTypeGraphicLabel(draft.graphic)}
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {wellTypeGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        disabled={submitting}
        variant="column"
        title="Select Well Type Graphic"
        subtitle="Select a graphic for the currently adding or updating well type."
        listAriaLabel="Well type graphics"
        closeAriaLabel="Close Select Well Type Graphic dialog"
        fallbackLabel={wellTypeGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Well Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected well type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
