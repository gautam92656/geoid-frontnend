...
import { listWaterObsGraphics } from "../../services/waterObsGraphicsApi";
import { SelectDrillingGraphicsModal } from "./SelectDrillingGraphicsModal";

type ManageWaterObservationTypesModalProps = Readonly<{
  open: boolean;
  options: WaterObservationTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WaterObservationTypeOption[];
  onClose: () => void;
  onSave: (options: WaterObservationTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Water Observation Type";

function optionToDraft(option: WaterObservationTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
      graphic: DEFAULT_WATER_OBSERVATION_GRAPHIC,
      depthRequired: true,
    };
  }
  ...
  return {
    ...
    graphic: option.graphic ?? DEFAULT_WATER_OBSERVATION_GRAPHIC,
    depthRequired: Boolean(option.depthRequired),
  };
}

function draftToOption(draft: DraftForm, id: string): WaterObservationTypeOption {
  return createBlankWaterObservationTypeOption({
    id,
    name: draft.name.trim(),
    tablogsAlias: draft.tablogsAlias.trim() || null,
    graphic: draft.graphic.trim() || DEFAULT_WATER_OBSERVATION_GRAPHIC,
    depthRequired: draft.depthRequired,
  });
}

function reorderEntries(
  entries: WaterObservationTypeOption[],
  sourceId: string,
  targetId: string
...
function validateName(
  ...
  if (!trimmed) return "Water observation type name is required.";
  ...
  if (duplicate) return "A water observation type with this name already exists.";
  return undefined;
}
...
export function ManageWaterObservationTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWaterObservationTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WaterObservationTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WaterObservationGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWaterObservationGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WATER_OBSERVATION_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );
  ...
  const resetPanel = useCallback(
    (
      ...
      adding: boolean,
      nextEntries: WaterObservationTypeOption[]
    ) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWaterObservationTypeOptions(options, []);
  ...
  useEffect(() => {
    ...
    listWaterObsGraphics()
      .then((catalog) => {
        if (cancelled) return;
        const next = (catalog.graphics ?? []).map((entry) => ({
          ...entry,
          url: getWaterObservationGraphicUrl(entry.filename) || entry.url,
          label: entry.label || waterObservationGraphicLabel(entry.filename),
        }));
        setGraphics(
          next.length > 0
            ? next
            : FALLBACK_WATER_OBS_GRAPHICS.map((filename) =>
                toWaterObservationGraphicCatalogEntry(filename)
              )
        );
      })
      .catch((error: unknown) => {
        ...
        // Keep picker usable with local fallback catalog; don't block the grid on API failure.
        setGraphicsError(null);
        console.warn(
          error instanceof Error ? error.message : "Failed to load water observation graphics."
        );
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WaterObservationTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} water observation types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("water-observation-type"))];
    }
  ...
  const handleSelectEntry = (entry: WaterObservationTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WaterObservationTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Water observation type name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Water Observation Type"
    : selectedEntry
      ? `Edit ${draft.name.trim() || selectedEntry.name}`
      : "Edit Water Observation Type";

  const listLabelForEntry = (entry: WaterObservationTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Water Observation Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-water-observation-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-water-observation-types-title" className="project-modal__title">
              Manage Water Observation Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing water observation types by selecting from the left menu. You can
              also drag to reorder. You also have the option to create a new Water Observation Type
              or copy an existing one used elsewhere in your company account.
            </p>
          </div>
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Water observation types"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">
                        No water observation types yet.
                      </li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new water observation type, or copy one used elsewhere in your
                      company account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Water Observation Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Water Observation Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Water Observation Type from scratch
                        </p>
                    ...
                    <div className="manage-origins-modal__row">
                      <FormField
                        label="Water Observation Type Name"
                        required
                        error={errors.name}
                        htmlFor={`${formId}-name`}
                      ...
                      >
                        <Input
                          ...
                          variant="ui"
                          type="text"
                          placeholder="Water Observation Type Name"
                          value={draft.name}
                          disabled={submitting}
                    ...
                    <div className="manage-water-obs-modal__graphic-block">
                      <span className="manage-insitu-modal__field-label">
                        Select Water Observation Graphic
                      </span>
                      <button
                        type="button"
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={waterObservationGraphicLabel(draft.graphic)}
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {waterObservationGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        disabled={submitting}
        variant="square"
        title="Select Water Observation Graphic"
        subtitle="Select a graphic for the currently adding or updating water observation type."
        listAriaLabel="Water observation graphics"
        closeAriaLabel="Close Select Water Observation Graphic dialog"
        fallbackLabel={waterObservationGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Water Observation Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected water observation type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
