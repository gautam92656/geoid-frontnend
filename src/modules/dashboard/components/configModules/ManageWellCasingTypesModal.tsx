...
} from "../../utils/configModules/wellCasingType";
...
type ManageWellCasingTypesModalProps = Readonly<{
  open: boolean;
  options: WellCasingTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WellCasingTypeOption[];
  onClose: () => void;
  onSave: (options: WellCasingTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Well Casing Type";

function optionToDraft(option: WellCasingTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
      type: "surface",
      graphic: DEFAULT_WELL_CASING_GRAPHIC,
  ...
  return {
    ...
    graphic: option.graphic ?? DEFAULT_WELL_CASING_GRAPHIC,
    allowNegativeDepth: Boolean(option.allowNegativeDepth),
  };
}

function draftToOption(draft: DraftForm, id: string): WellCasingTypeOption {
  return createBlankWellCasingTypeOption({
    id,
    name: draft.name.trim(),
    tablogsAlias: draft.tablogsAlias.trim() || null,
    type: draft.type,
    graphic: draft.graphic.trim() || DEFAULT_WELL_CASING_GRAPHIC,
    allowNegativeDepth: draft.allowNegativeDepth,
  });
}

function reorderEntries(
  entries: WellCasingTypeOption[],
  sourceId: string,
  targetId: string
...
function validateName(
  ...
  if (!trimmed) return "Well casing type name is required.";
  ...
  if (duplicate) return "A well casing type with this name already exists.";
  return undefined;
}
...
export function ManageWellCasingTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWellCasingTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WellCasingTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WellCasingGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWellCasingGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WELL_CASING_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );

  const kindSelectOptions = useMemo(
    () => WELL_CASING_KIND_OPTIONS.map((entry) => ({ ...entry })),
    []
  );
  ...
  const resetPanel = useCallback(
    (
      ...
      adding: boolean,
      nextEntries: WellCasingTypeOption[]
    ) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWellCasingTypeOptions(options, []);
    ...
    listWellCasingGraphics()
      .then((catalog) => {
        ...
        const next = (catalog.graphics ?? []).map((entry) =>
          toWellCasingGraphicCatalogEntry(entry.filename, entry.label)
        );
        setGraphics(
          next.length > 0
            ? next
            : FALLBACK_WELL_CASING_GRAPHICS.map((filename) =>
                toWellCasingGraphicCatalogEntry(filename)
              )
        );
      })
      .catch((error: unknown) => {
        ...
        setGraphicsError(null);
        console.warn(
          error instanceof Error ? error.message : "Failed to load well casing graphics."
        );
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WellCasingTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} well casing types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("well-casing-type"))];
    }
  ...
  const handleSelectEntry = (entry: WellCasingTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WellCasingTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Well casing type name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Well Casing Type"
    : selectedEntry
      ? `Edit ${draft.name.trim() || selectedEntry.name}`
      : "Edit Well Casing Type";

  const listLabelForEntry = (entry: WellCasingTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Well Casing Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-well-casing-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-well-casing-types-title" className="project-modal__title">
              Manage Well Casing Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing well casing types by selecting from the left menu. You can also
              drag to reorder. You also have the option to create a new Well Casing Type or copy an
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Well casing types"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">No well casing types yet.</li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new well casing type, or copy one used elsewhere in your company
                      account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Well Casing Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Well Casing Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Well Casing Type from scratch
                        </p>
                    ...
                    <div className="manage-origins-modal__row">
                      ...
                      >
                        <Input
                          ...
                          variant="ui"
                          type="text"
                          placeholder="Well Casing Type Name"
                          value={draft.name}
                          disabled={submitting}
                    ...
                    >
                      <Select
                        ...
                        onChange={(value) =>
                          patchDraft({
                            type: value === "regular" ? "regular" : "surface",
                          })
                        }
                      />
                    </FormField>
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={wellCasingGraphicLabel(draft.graphic)}
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {wellCasingGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        subtitle={`Select a graphic from ${WELL_CASING_GRAPHICS_API_BASE.replace("/api/v1/", "")}.`}
        listAriaLabel="Well casing graphics"
        closeAriaLabel="Close Select Well Casing Graphic dialog"
        fallbackLabel={wellCasingGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Well Casing Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected well casing type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
