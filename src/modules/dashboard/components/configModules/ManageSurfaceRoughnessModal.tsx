...
const NEW_ROUGHNESS_LABEL = "New Surface Roughness";
const CODE_MAX_LENGTH = 32;

type ManageSurfaceRoughnessModalProps = Readonly<{
  open: boolean;
  options: SurfaceRoughnessOption[];
  onClose: () => void;
  onSave: (options: SurfaceRoughnessOption[]) => void | Promise<void>;
}>;
...
function optionToDraft(option: SurfaceRoughnessOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      code: "",
...
function draftToOption(draft: DraftForm, id: string): SurfaceRoughnessOption {
  return createBlankSurfaceRoughnessOption({
    id,
...
function reorderEntries(
  entries: SurfaceRoughnessOption[],
  sourceId: string,
  targetId: string
...
function validateName(
  ...
  if (!trimmed) return "Surface roughness name is required.";
  ...
  if (duplicate) return "A surface roughness with this name already exists.";
  return undefined;
}
...
export function ManageSurfaceRoughnessModal({
  open,
  options,
  onClose,
  onSave,
}: ManageSurfaceRoughnessModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<SurfaceRoughnessOption[]>([]);
  ...
  const resetPanel = useCallback(
    (nextSelectedId: string | null, adding: boolean, nextEntries: SurfaceRoughnessOption[]) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseSurfaceRoughnessOptions(options, []);
  ...
  const commitDraft = useCallback((): SurfaceRoughnessOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} surface roughness values.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("surface-roughness"))];
    }
  ...
  const handleSelectEntry = (entry: SurfaceRoughnessOption) => {
  ...
  const persistEntries = async (nextEntries: SurfaceRoughnessOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Surface roughness name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Surface Roughness"
    : selectedEntry
      ? `Edit ${selectedEntry.name}`
      : "Edit Surface Roughness";
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Surface Roughness dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-surface-roughness-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-surface-roughness-title" className="project-modal__title">
              Manage Surface Roughness
            </h2>
            <p className="project-modal__subtitle">
              Manage the surface roughness values which are made available by this log
              configuration.
            </p>
          </div>
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  <div className="project-modal__split-sidebar-head">
                    <span className="project-modal__split-sidebar-label">Surface Roughness</span>
                    <UiButton
                      type="button"
                      variant="primary"
                      size="sm"
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Surface roughness options"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">
                        No surface roughness values yet.
                      </li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                          ...
                          <li className="project-modal__type-list-item">
                            <div className="project-modal__type-item is-selected">
                              ...
                              <span
                                ...
                                aria-selected="true"
                              >
                                {draft.name.trim() || NEW_ROUGHNESS_LABEL}
                              </span>
                            </div>
                          </li>
                        ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack">
                    ...
                    >
                      <Input
                        ...
                        variant="ui"
                        type="text"
                        placeholder="Surface Roughness Name"
                        value={draft.name}
                        disabled={submitting}
      ...
      <ConfirmDialog
        ...
        title="Delete Surface Roughness"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected surface roughness. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
