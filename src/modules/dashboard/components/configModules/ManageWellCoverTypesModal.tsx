...
} from "../../utils/configModules/wellCoverType";
import { listWellCoverGraphics } from "../../services/wellCoverGraphicsApi";
import { SelectDrillingGraphicsModal } from "./SelectDrillingGraphicsModal";

type ManageWellCoverTypesModalProps = Readonly<{
  open: boolean;
  options: WellCoverTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WellCoverTypeOption[];
  onClose: () => void;
  onSave: (options: WellCoverTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Well Cover Type";

function optionToDraft(option: WellCoverTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
      graphic: DEFAULT_WELL_COVER_GRAPHIC,
      allowNegativeDepth: false,
      graphicAlignment: DEFAULT_WELL_COVER_GRAPHIC_ALIGNMENT,
    };
  }
  ...
  return {
    ...
    graphic: option.graphic ?? DEFAULT_WELL_COVER_GRAPHIC,
...
function draftToOption(draft: DraftForm, id: string): WellCoverTypeOption {
  return createBlankWellCoverTypeOption({
    id,
    name: draft.name.trim(),
    tablogsAlias: draft.tablogsAlias.trim() || null,
    graphic: draft.graphic.trim() || DEFAULT_WELL_COVER_GRAPHIC,
    allowNegativeDepth: draft.allowNegativeDepth,
    graphicAlignment: draft.graphicAlignment,
  });
}

function reorderEntries(
  entries: WellCoverTypeOption[],
  sourceId: string,
  targetId: string
): WellCoverTypeOption[] {
...
function validateName(
  ...
  if (!trimmed) return "Well cover type name is required.";
  ...
  if (duplicate) return "A well cover type with this name already exists.";
  return undefined;
}
...
export function ManageWellCoverTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWellCoverTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WellCoverTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WellCoverGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWellCoverGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WELL_COVER_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );

  const fallbackGraphics = useMemo(
    () => FALLBACK_WELL_COVER_GRAPHICS.map((filename) => toWellCoverGraphicCatalogEntry(filename)),
    []
  );

  const resetPanel = useCallback(
    (nextSelectedId: string | null, adding: boolean, nextEntries: WellCoverTypeOption[]) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWellCoverTypeOptions(options, []);
  ...
  useEffect(() => {
    ...
    listWellCoverGraphics()
      .then((catalog) => {
        if (cancelled) return;
        const fromApi = (catalog.graphics ?? []).map((entry) =>
          toWellCoverGraphicCatalogEntry(entry.filename, entry.label)
        );
        setGraphics(fromApi.length > 0 ? fromApi : fallbackGraphics);
        setGraphicsError(null);
      })
      .catch((error: unknown) => {
        if (cancelled) return;
        setGraphics(fallbackGraphics);
        setGraphicsError(null);
        console.warn(
          error instanceof Error ? error.message : "Failed to load well cover graphics."
        );
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WellCoverTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} well cover types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("well-cover"))];
    }
  ...
  const handleSelectEntry = (entry: WellCoverTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WellCoverTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Well cover type name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Well Cover Type"
    : selectedEntry
      ? `Edit ${draft.name.trim() || selectedEntry.name}`
      : "Edit Well Cover Type";

  const listLabelForEntry = (entry: WellCoverTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Well Cover Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-well-cover-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-well-cover-types-title" className="project-modal__title">
              Manage Well Cover Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing well cover types by selecting from the left menu. You can also
              drag to reorder. You also have the option to create a new Well Cover Type or copy an
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Well cover types"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">No well cover types yet.</li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new well cover type, or copy one used elsewhere in your company
                      account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Well Cover Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Well Cover Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Well Cover Type from scratch
                        </p>
                    ...
                    <div className="manage-origins-modal__row">
                      ...
                      >
                        <Input
                          ...
                          variant="ui"
                          type="text"
                          placeholder="Well Cover Type Name"
                          value={draft.name}
                          disabled={submitting}
                    ...
                    <div className="manage-insitu-modal__settings">
                      <div className="manage-insitu-modal__toggle-inline">
                        <Toggle
                        ...
                        />
                        <label htmlFor={`${formId}-negative-depth`}>Allow negative depth</label>
                      </div>
                      <p className="manage-origins-modal__hint">
                        Negative depths allow you to display well logs above the ground
                      </p>
                    </div>

                    <div className="manage-drilling-modal__log-kind">
                      <span className="manage-insitu-modal__field-label">Graphic Alignment</span>
                      <div
                        className="manage-drilling-modal__log-kind-group"
                        role="radiogroup"
                        aria-label="Graphic Alignment"
                      >
                        <button
                          type="button"
                          role="radio"
                          aria-checked={draft.graphicAlignment === "top"}
                          className={[
                            "manage-drilling-modal__log-kind-option",
                            draft.graphicAlignment === "top" ? "is-selected" : "",
                          ]
                            .filter(Boolean)
                            .join(" ")}
                          disabled={submitting}
                          onClick={() => patchDraft({ graphicAlignment: "top" })}
                        >
                          Top
                        </button>
                        <button
                          type="button"
                          role="radio"
                          aria-checked={draft.graphicAlignment === "bottom"}
                          className={[
                            "manage-drilling-modal__log-kind-option",
                            draft.graphicAlignment === "bottom" ? "is-selected" : "",
                          ]
                            .filter(Boolean)
                            .join(" ")}
                          disabled={submitting}
                          onClick={() => patchDraft({ graphicAlignment: "bottom" })}
                        >
                          Bottom
                        </button>
                      </div>
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={wellCoverGraphicLabel(draft.graphic)}
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {wellCoverGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        disabled={submitting}
        variant="column"
        title="Select Well Cover Graphic"
        subtitle="Select a graphic for the currently adding or updating well cover type."
        listAriaLabel="Well cover graphics"
        closeAriaLabel="Close Select Well Cover Graphic dialog"
        fallbackLabel={wellCoverGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Well Cover Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected well cover type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
