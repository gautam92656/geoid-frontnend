...
import { listWellCasingGraphics } from "../../services/wellCasingGraphicsApi";
import { SelectDrillingGraphicsModal } from "./SelectDrillingGraphicsModal";

type ManageWellCasingTopTypesModalProps = Readonly<{
  open: boolean;
  options: WellCasingTopTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WellCasingTopTypeOption[];
  onClose: () => void;
  onSave: (options: WellCasingTopTypeOption[]) => void | Promise<void>;
}>;
...
const NEW_TYPE_LABEL = "New Well Casing Top Type";

function optionToDraft(option: WellCasingTopTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
  ...
  return {
    ...
    graphic: option.graphic ?? DEFAULT_WELL_CASING_TOP_GRAPHIC,
    allowNegativeDepth: Boolean(option.allowNegativeDepth),
  };
}

function draftToOption(draft: DraftForm, id: string): WellCasingTopTypeOption {
  return createBlankWellCasingTopTypeOption({
    id,
    name: draft.name.trim(),
    tablogsAlias: draft.tablogsAlias.trim() || null,
    graphic: draft.graphic.trim() || DEFAULT_WELL_CASING_TOP_GRAPHIC,
    allowNegativeDepth: draft.allowNegativeDepth,
  });
}

function reorderEntries(
  entries: WellCasingTopTypeOption[],
  sourceId: string,
  targetId: string
...
function validateName(
  ...
  if (!trimmed) return "Well casing top type name is required.";
  ...
  if (duplicate) return "A well casing top type with this name already exists.";
  return undefined;
}
...
export function ManageWellCasingTopTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWellCasingTopTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WellCasingTopTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WellCasingTopGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWellCasingTopGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WELL_CASING_TOP_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );

  const fallbackGraphics = useMemo(
    () =>
      FALLBACK_WELL_CASING_TOP_GRAPHICS.map((filename) =>
        toWellCasingTopGraphicCatalogEntry(filename)
      ),
    []
  );
  ...
  const resetPanel = useCallback(
    (
      ...
      adding: boolean,
      nextEntries: WellCasingTopTypeOption[]
    ) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWellCasingTopTypeOptions(options, []);
  ...
  useEffect(() => {
    ...
    listWellCasingGraphics()
      .then((catalog) => {
        if (cancelled) return;
        const fromApi = (catalog.graphics ?? []).map((entry) =>
          toWellCasingTopGraphicCatalogEntry(entry.filename, entry.label)
        );
      ...
      .catch((error: unknown) => {
        ...
        setGraphicsError(null);
        console.warn(
          error instanceof Error ? error.message : "Failed to load well casing top graphics."
        );
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WellCasingTopTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} well casing top types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("well-casing-top"))];
    }
  ...
  const handleSelectEntry = (entry: WellCasingTopTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WellCasingTopTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Well casing top type name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Well Casing Top Type"
    : selectedEntry
      ? `Edit ${draft.name.trim() || selectedEntry.name}`
      : "Edit Well Casing Top";

  const listLabelForEntry = (entry: WellCasingTopTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Well Casing Top Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-well-casing-top-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-well-casing-top-types-title" className="project-modal__title">
              Manage Well Casing Top Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing well casing top types by selecting from the left menu. You can
              also drag to reorder. You also have the option to create a new Well Casing Top Type or
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Well casing top types"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">
                        No well casing top types yet.
                      </li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new well casing top type, or copy one used elsewhere in your company
                      account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Well Casing Top Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Well Casing Top Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Well Casing Top Type from scratch
                        </p>
                    ...
                    <div className="manage-origins-modal__row">
                      <FormField
                        label="Well Casing Top Type Name"
                        required
                        error={errors.name}
                        htmlFor={`${formId}-name`}
                      ...
                      >
                        <Input
                          ...
                          variant="ui"
                          type="text"
                          placeholder="Well Casing Top Type Name"
                          value={draft.name}
                          disabled={submitting}
                    ...
                    <div className="manage-insitu-modal__settings manage-well-casing-modal__negative-depth">
                      <label
                        className="manage-insitu-modal__toggle-inline"
                        htmlFor={`${formId}-allow-negative-depth`}
                      >
                        <Toggle
                          id={`${formId}-allow-negative-depth`}
                      ...
                      <span className="manage-well-casing-modal__hint">
                    ...
                    <div className="manage-water-obs-modal__graphic-block">
                      <span className="manage-insitu-modal__field-label">
                        Select Well Casing Graphic
                      </span>
                      <button
                        type="button"
                        className="manage-insitu-modal__graphic-card manage-water-obs-modal__graphic-card"
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={wellCasingTopGraphicLabel(draft.graphic)}
                            className="manage-water-obs-modal__graphic-image"
                          />
                        ) : (
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {wellCasingTopGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        disabled={submitting}
        variant="square"
        title="Select Well Casing Graphic"
        subtitle={`Select a graphic from ${WELL_CASING_TOP_GRAPHICS_API_BASE.replace("/api/v1/", "")}.`}
        listAriaLabel="Well casing top graphics"
        closeAriaLabel="Close Select Well Casing Graphic dialog"
        fallbackLabel={wellCasingTopGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Well Casing Top Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected well casing top type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
