...
import { listWellBackfillGraphics } from "../../services/wellBackfillGraphicsApi";
import { SelectDrillingGraphicsModal } from "./SelectDrillingGraphicsModal";

type ManageWellBackfillTypesModalProps = Readonly<{
  open: boolean;
  options: WellBackfillTypeOption[];
  /** Company-wide types available to copy when adding. */
  companyOptions?: WellBackfillTypeOption[];
  onClose: () => void;
  onSave: (options: WellBackfillTypeOption[]) => void | Promise<void>;
}>;
...
function optionToDraft(option: WellBackfillTypeOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      tablogsAlias: "",
...
function draftToOption(draft: DraftForm, id: string): WellBackfillTypeOption {
...
function reorderEntries(
  entries: WellBackfillTypeOption[],
  sourceId: string,
  targetId: string
...
function validateName(
  ...
  if (!trimmed) return "Well backfill type name is required.";
  ...
  if (duplicate) return "A well backfill type with this name already exists.";
  return undefined;
}
...
export function ManageWellBackfillTypesModal({
  open,
  options,
  companyOptions = [],
  onClose,
  onSave,
}: ManageWellBackfillTypesModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<WellBackfillTypeOption[]>([]);
  ...
  const [graphics, setGraphics] = useState<WellBackfillGraphicCatalogEntry[]>([]);
  ...
  const selectedGraphicUrl = getWellBackfillGraphicUrl(draft.graphic);
  ...
  const aliasSelectOptions = useMemo(
    () => WELL_BACKFILL_TABLOGS_ALIAS_OPTIONS.map((entry) => ({ ...entry })),
    []
  );
  ...
  const resetPanel = useCallback(
    (
      ...
      adding: boolean,
      nextEntries: WellBackfillTypeOption[]
    ) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseWellBackfillTypeOptions(options, []);
  ...
  useEffect(() => {
    ...
    listWellBackfillGraphics()
      .then((catalog) => {
        if (cancelled) return;
        const next = (catalog.graphics ?? []).map((entry) =>
          toWellBackfillGraphicCatalogEntry(entry.filename, entry.label)
        );
        setGraphics(
          next.length > 0
            ? next
            : FALLBACK_WELL_BACKFILL_GRAPHICS.map((filename) =>
                toWellBackfillGraphicCatalogEntry(filename)
              )
        );
      })
      .catch((error: unknown) => {
        ...
        setGraphicsError(null);
        console.warn(
          error instanceof Error ? error.message : "Failed to load well backfill graphics."
        );
      })
      .finally(() => {
  ...
  const commitDraft = useCallback((): WellBackfillTypeOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} well backfill types.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("well-backfill-type"))];
    }
  ...
  const handleSelectEntry = (entry: WellBackfillTypeOption) => {
  ...
  const persistEntries = async (nextEntries: WellBackfillTypeOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Well backfill type name is required." });
        return;
      }
  ...
  const panelTitle = isAdding
    ? "Add New Well Backfill Type"
    : selectedEntry
      ? `Edit ${draft.name.trim() || selectedEntry.name}`
      : "Edit Well Backfill Type";

  const listLabelForEntry = (entry: WellBackfillTypeOption) => {
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Well Backfill Types dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-well-backfill-types-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-well-backfill-types-title" className="project-modal__title">
              Manage Well Backfill Types
            </h2>
            <p className="project-modal__subtitle">
              Manage your existing well backfill types by selecting from the left menu. You can also
              drag to reorder. You also have the option to create a new Well Backfill Type or copy an
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Well backfill types"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">No well backfill types yet.</li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                ...
                <div className="project-modal__split-main">
                  ...
                  {isAdding ? (
                    <p className="project-modal__split-main-subtitle">
                      Create a new well backfill type, or copy one used elsewhere in your company
                      account.
                    </p>
                  ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack manage-origins-modal__fields">
                    {isAdding && companySelectOptions.length > 0 ? (
                      <>
                        <FormField
                          label="Select Existing Well Backfill Type"
                        ...
                        >
                          <Select
                            ...
                            placeholder="Select Existing Well Backfill Type"
                            onChange={handleCompanyCopy}
                          />
                        </FormField>

                        <p className="manage-origins-modal__hint">
                          or - create a new Well Backfill Type from scratch
                        </p>
                    ...
                    <div className="manage-origins-modal__row">
                      ...
                      >
                        <Input
                          ...
                          variant="ui"
                          type="text"
                          placeholder="Well Backfill Type Name"
                          value={draft.name}
                          disabled={submitting}
                      ...
                      >
                        {selectedGraphicUrl ? (
                          ...
                          <img
                            ...
                            alt={wellBackfillGraphicLabel(draft.graphic)}
                        ...
                        <span className="manage-insitu-modal__graphic-card-title">
                          {wellBackfillGraphicLabel(draft.graphic)}
                        </span>
                      </button>
                    </div>
      ...
      <SelectDrillingGraphicsModal
        ...
        disabled={submitting}
        variant="square"
        title="Select Well Backfill Graphic"
        subtitle={`Select a graphic from ${WELL_BACKFILL_GRAPHICS_API_BASE.replace("/api/v1/", "")}.`}
        listAriaLabel="Well backfill graphics"
        closeAriaLabel="Close Select Well Backfill Graphic dialog"
        fallbackLabel={wellBackfillGraphicLabel}
      ...
      <ConfirmDialog
        ...
        title="Delete Well Backfill Type"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected well backfill type. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
