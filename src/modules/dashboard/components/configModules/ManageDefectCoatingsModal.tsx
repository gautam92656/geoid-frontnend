...
} from "../../utils/configModules/defectCoating";

const NEW_COATING_LABEL = "New Defect Coating";
const CODE_MAX_LENGTH = 32;

type ManageDefectCoatingsModalProps = Readonly<{
  open: boolean;
  options: DefectCoatingOption[];
  onClose: () => void;
  onSave: (options: DefectCoatingOption[]) => void | Promise<void>;
}>;
...
function optionToDraft(option: DefectCoatingOption | null): DraftForm {
  if (!option) {
    return {
      name: "",
      code: "",
...
function draftToOption(draft: DraftForm, id: string): DefectCoatingOption {
  return createBlankDefectCoatingOption({
    id,
...
function validateName(
  ...
  if (!trimmed) return "Defect Coating name is required.";
  ...
  if (duplicate) return "A Defect Coating with this name already exists.";
  return undefined;
}
...
export function ManageDefectCoatingsModal({
  open,
  options,
  onClose,
  onSave,
}: ManageDefectCoatingsModalProps) {
  const formId = useId();
  const [entries, setEntries] = useState<DefectCoatingOption[]>([]);
  ...
  const resetPanel = useCallback(
    (nextSelectedId: string | null, adding: boolean, nextEntries: DefectCoatingOption[]) => {
  ...
  useEffect(() => {
    ...
    const nextEntries = parseDefectCoatingOptions(options, []);
  ...
  const commitDraft = useCallback((): DefectCoatingOption[] | null => {
    ...
    if (isAdding) {
      if (entries.length >= MODULE_OPTIONS_MAX_COUNT) {
        setErrors({
          name: `You can add up to ${MODULE_OPTIONS_MAX_COUNT} defect coatings.`,
        });
        return null;
      }
      return [...entries, draftToOption(draft, createOptionId("defect-coating"))];
    }
  ...
  const handleSelectEntry = (entry: DefectCoatingOption) => {
  ...
  const persistEntries = async (nextEntries: DefectCoatingOption[]) => {
    setSubmitting(true);
    try {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (discardIncompleteAdd) {
      if (entries.length === 0) {
        setErrors({ name: "Defect Coating name is required." });
        return;
      }
    ...
    <ProjectModalPortal open={open}>
      <div className="project-modal project-modal--stacked project-modal--nested" role="presentation">
        <button
          ...
          aria-label="Close Manage Defect Coatings dialog"
          onClick={onClose}
        />
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="manage-defect-coatings-title"
        >
          <div className="project-modal__header">
            <h2 id="manage-defect-coatings-title" className="project-modal__title">
              Manage Defect Coatings
            </h2>
            <p className="project-modal__subtitle">
              Manage the Defect Coating values which are made available by this log
              configuration.
            </p>
          </div>
              ...
              <div className="project-modal__split">
                <aside className="project-modal__split-sidebar">
                  <div className="project-modal__split-sidebar-head">
                    <span className="project-modal__split-sidebar-label">Defect Coatings</span>
                    <UiButton
                      type="button"
                      variant="primary"
                      size="sm"
                  ...
                  <ul
                    ...
                    role="listbox"
                    aria-label="Defect coating options"
                  >
                    {entries.length === 0 && !isAdding ? (
                      <li className="project-modal__type-list-empty">No defect coatings yet.</li>
                    ) : (
                      <>
                        {entries.map((entry) => {
                          ...
                          <li className="project-modal__type-list-item">
                            <div className="project-modal__type-item is-selected">
                              ...
                              <span
                                ...
                                aria-selected="true"
                              >
                                {draft.name.trim() || NEW_COATING_LABEL}
                              </span>
                            </div>
                          </li>
                        ) : null}
                  ...
                  <div className="project-modal__fields project-modal__fields--stack">
                    ...
                    >
                      <Input
                        ...
                        variant="ui"
                        type="text"
                        placeholder="Defect Coating Name"
                        value={draft.name}
                        disabled={submitting}
      ...
      <ConfirmDialog
        ...
        title="Delete Defect Coating"
        message={
          selectedEntry
            ? `This will remove "${selectedEntry.name}". You must save to apply this change.`
            : "This will remove the selected defect coatings. You must save to apply this change."
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
