...
import {
  getDefectOpennessTemplates,
  getUserDefectOpennesses,
  resetUserDefectOpennesses,
  saveUserDefectOpennesses,
} from "../services/configModulesApi";

type UseUserDefectOpennessesOptions = {
  enabled?: boolean;
...
/**
 * Loads and persists defect openness values for a specific log configuration,
 * seeded from common templates.
 */
export function useUserDefectOpennesses(
  moduleSlug: string,
  options: UseUserDefectOpennessesOptions
) {
  const { enabled = true, logConfigurationId } = options;
  const canLoad = enabled && hasLogConfigurationId(logConfigurationId);
  const [items, setItems] = useState<DefectOpennessOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const latestRef = useRef<DefectOpennessOption[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const { data } = await getUserDefectOpennesses(moduleSlug, logConfigurationId);
      const parsed = parseDefectOpennessOptions(data, []);
  ...
  const save = useCallback(
    async (next: DefectOpennessOption[]) => {
      if (!hasLogConfigurationId(logConfigurationId)) return [];
      setSaving(true);
      try {
        const { data } = await saveUserDefectOpennesses(
          moduleSlug,
          next,
          logConfigurationId
        );
        const saved = parseDefectOpennessOptions(data, next);
  ...
  const resetToTemplate = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { data } = await resetUserDefectOpennesses(moduleSlug, logConfigurationId);
      const reset = parseDefectOpennessOptions(data, []);
  ...
  const loadTemplate = useCallback(async () => {
    const { data } = await getDefectOpennessTemplates(moduleSlug);
    return parseDefectOpennessOptions(data, []);
  }, [moduleSlug]);
