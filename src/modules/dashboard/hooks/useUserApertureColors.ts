...
import {
  getApertureColorTemplates,
  getUserApertureColors,
  resetUserApertureColors,
  saveUserApertureColors,
} from "../services/configModulesApi";

type UseUserApertureColorsOptions = {
  enabled?: boolean;
...
/**
 * Loads and persists aperture colors for a specific log configuration,
 * seeded from common templates.
 */
export function useUserApertureColors(
  moduleSlug: string,
  options: UseUserApertureColorsOptions
) {
  const { enabled = true, logConfigurationId } = options;
  const canLoad = enabled && hasLogConfigurationId(logConfigurationId);
  const [items, setItems] = useState<ColorOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const latestRef = useRef<ColorOption[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const { data } = await getUserApertureColors(moduleSlug, logConfigurationId);
      const parsed = parseColorOptions(data, []);
  ...
  const save = useCallback(
    async (next: ColorOption[]) => {
      if (!hasLogConfigurationId(logConfigurationId)) return [];
      setSaving(true);
      try {
        const { data } = await saveUserApertureColors(moduleSlug, next, logConfigurationId);
        const saved = parseColorOptions(data, next);
  ...
  const resetToTemplate = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { data } = await resetUserApertureColors(moduleSlug, logConfigurationId);
      const reset = parseColorOptions(data, []);
  ...
  const loadTemplate = useCallback(async () => {
    const { data } = await getApertureColorTemplates(moduleSlug);
    return parseColorOptions(data, []);
  }, [moduleSlug]);
