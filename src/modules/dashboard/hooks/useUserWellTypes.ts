...
import {
  getWellTypeTemplates,
  getUserWellTypes,
  resetUserWellTypes,
  saveUserWellTypes,
} from "../services/configModulesApi";

type UseUserWellTypesOptions = {
  enabled?: boolean;
...
/**
 * Loads and persists well types for a specific log configuration,
 * seeded from common templates.
 */
export function useUserWellTypes(moduleSlug: string, options: UseUserWellTypesOptions) {
  const { enabled = true, logConfigurationId } = options;
  const canLoad = enabled && hasLogConfigurationId(logConfigurationId);
  const [items, setItems] = useState<WellTypeOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const latestRef = useRef<WellTypeOption[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const { data } = await getUserWellTypes(moduleSlug, logConfigurationId);
      const parsed = parseWellTypeOptions(data, []);
  ...
  const save = useCallback(
    async (next: WellTypeOption[]) => {
      if (!hasLogConfigurationId(logConfigurationId)) return [];
      setSaving(true);
      try {
        const { data } = await saveUserWellTypes(moduleSlug, next, logConfigurationId);
        const saved = parseWellTypeOptions(data, next);
  ...
  const resetToTemplate = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { data } = await resetUserWellTypes(moduleSlug, logConfigurationId);
      const reset = parseWellTypeOptions(data, []);
  ...
  const loadTemplate = useCallback(async () => {
    const { data } = await getWellTypeTemplates(moduleSlug);
    return parseWellTypeOptions(data, []);
  }, [moduleSlug]);
