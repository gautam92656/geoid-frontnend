...
import {
  ...
  resetUserDrillingTypes,
  saveUserDrillingTypes,
} from "../services/configModulesApi";

type UseUserDrillingTypesOptions = {
  enabled?: boolean;
...
/**
 * Loads and persists drilling types for a specific log configuration,
 * seeded from common templates.
 */
export function useUserDrillingTypes(
  moduleSlug: string,
  options: UseUserDrillingTypesOptions
) {
  const { enabled = true, logConfigurationId } = options;
  const canLoad = enabled && hasLogConfigurationId(logConfigurationId);
  const [items, setItems] = useState<DrillingTypeOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const latestRef = useRef<DrillingTypeOption[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const { data } = await getUserDrillingTypes(moduleSlug, logConfigurationId);
      const parsed = parseDrillingTypeOptions(data, []);
  ...
  const save = useCallback(
    async (next: DrillingTypeOption[]) => {
      if (!hasLogConfigurationId(logConfigurationId)) return [];
      setSaving(true);
      try {
        const { data } = await saveUserDrillingTypes(moduleSlug, next, logConfigurationId);
        const saved = parseDrillingTypeOptions(data, next);
  ...
  const resetToTemplate = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { data } = await resetUserDrillingTypes(moduleSlug, logConfigurationId);
      const reset = parseDrillingTypeOptions(data, []);
  ...
  const loadTemplate = useCallback(async () => {
    const { data } = await getDrillingTypeTemplates(moduleSlug);
    return parseDrillingTypeOptions(data, []);
  }, [moduleSlug]);
