...
import {
  ...
  resetUserWaterObservationTypes,
  saveUserWaterObservationTypes,
} from "../services/configModulesApi";

type UseUserWaterObservationTypesOptions = {
  enabled?: boolean;
...
/**
 * Loads and persists water observation types for a specific log configuration,
 * seeded from common templates.
 */
export function useUserWaterObservationTypes(
  moduleSlug: string,
  options: UseUserWaterObservationTypesOptions
) {
  const { enabled = true, logConfigurationId } = options;
  const canLoad = enabled && hasLogConfigurationId(logConfigurationId);
  const [items, setItems] = useState<WaterObservationTypeOption[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const latestRef = useRef<WaterObservationTypeOption[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const { data } = await getUserWaterObservationTypes(moduleSlug, logConfigurationId);
      const parsed = parseWaterObservationTypeOptions(data, []);
  ...
  const save = useCallback(
    async (next: WaterObservationTypeOption[]) => {
      if (!hasLogConfigurationId(logConfigurationId)) return [];
      setSaving(true);
      try {
        const { data } = await saveUserWaterObservationTypes(moduleSlug, next, logConfigurationId);
        const saved = parseWaterObservationTypeOptions(data, next);
  ...
  const resetToTemplate = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const { data } = await resetUserWaterObservationTypes(moduleSlug, logConfigurationId);
      const reset = parseWaterObservationTypeOptions(data, []);
  ...
  const loadTemplate = useCallback(async () => {
    const { data } = await getWaterObservationTypeTemplates(moduleSlug);
    return parseWaterObservationTypeOptions(data, []);
  }, [moduleSlug]);
